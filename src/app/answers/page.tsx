...
export default function Answers() {
  ...
  const [companies, setCompanies] = useState<CompanyQAData[]>([]);
  ...
  const handleCopyAll = async () => {
    ...
    try {
      // Format all company Q&A data for copying
      const formattedData = companies.map((company, index) => {
        const qaText = company.qaList.map((qa, qaIndex) => 
...
${qaText}`;
      ...
      const finalText = `Y Combinator Application Q&As (${companies.length} companies)
${'='.repeat(60)}
...
${formattedData}`;
  ...
  const handleScrape = async () => {
    ...
    try {
      const response = await fetch('/api/scrape-qa', {
        method: 'POST',
        headers: {
      ...
      <div className="bg-white rounded-lg shadow-xl p-8 w-full max-w-4xl">
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold text-gray-800 mb-2">
            Y Combinator Application Q&A Scraper
          </h1>
          <p className="text-gray-600">
            Enter a Y Combinator companies URL to scrape application questions and answers
          </p>
        </div>
        ...
        >
          {isLoading ? (
            <div className="flex items-center justify-center">
              ...
              {progress ? progress.message : 'Scraping Q&As...'}
            </div>
          ) : (
            'Scrape Q&As'
          )}
              ...
              {companies.map((company, index) => (
                <div key={index} className="bg-gray-50 rounded-lg p-6 hover:bg-gray-100 transition-colors">
                  ...
                  <div className="space-y-4">
                    {company.qaList.map((qa, qaIndex) => (
                      <div key={qaIndex} className="border-l-4 border-orange-200 pl-4">
